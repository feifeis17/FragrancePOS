...
public class MainFrame extends JFrame {
    ...
    private boolean isScreensaverAktif = false;

    public MainFrame() {
        setTitle("Decium Perfumery - Point of Sales");
        setSize(1100, 680); 
    ...
    private void initScreensaver() {
        java.net.URL gifUrl = getClass().getResource("/com/fragrance/resources/icons/promo_decium.gif");
        Image gifImage = (gifUrl != null) ? new ImageIcon(gifUrl).getImage() : null;

        screensaverPanel = new JPanel(new BorderLayout()) {
            @Override
        ...
        if (gifImage == null) {
            JLabel lblFallback = new JLabel("DECIUM LUXURY FRAGRANCE", SwingConstants.CENTER);
            lblFallback.setForeground(ThemeConfig.ACCENT);
...
//fitur screensaver
    ...
    private void jalankanScreensaver() {
        isScreensaverAktif = true;
        screensaverPanel.setVisible(true);
    }

    private void tutupScreensaver() {
        isScreensaverAktif = false;
...
// Logout
        ...
        btnLogout.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) { btnLogout.setForeground(ThemeConfig.DANGER); }
            @Override public void mouseExited(MouseEvent e) { btnLogout.setForeground(ThemeConfig.DANGER); }
        });
...
//Logo di pojok kanan
        ...
        if(logoUrl != null) {
            ...
            lblLogoAtas.setIcon(new ImageIcon(img));
        }

        headerBar.add(titleStack, BorderLayout.WEST);
        headerBar.add(lblLogoAtas, BorderLayout.EAST); 
